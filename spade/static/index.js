...
let anyChartInstance = null; // Global variable to hold the chart instance
...
const tablesOptions = {
  ...
  "initComplete": function () {
    ...
    table.on('column-reorder', function (e, settings, details) {
      console.log('Column reorderering started:', details);
      // disable tooltips to prevent them from showing while reordering columns
      $('[data-bs-toggle="tooltip"]').tooltip('dispose');
    });

    table.on('columns-reordered', function (e, details) {
      console.log('Column reordered');
      tableColumnsState[tabName] = [];
    ...
    let tableButtons = tableContainer.querySelector('.dt-buttons');

    // Check if the JSON export button already exists
    let existingJsonExportButton = tableButtons.querySelector('.buttons-json');

    if (!existingJsonExportButton) {
      let jsonExportButton = document.createElement('button');
      jsonExportButton.className = 'btn btn-secondary buttons-json';
      jsonExportButton.setAttribute('tabindex', '0');
      jsonExportButton.setAttribute('aria-controls', tableContainer.querySelector('table').id);
      jsonExportButton.setAttribute('type', 'button');
      jsonExportButton.innerHTML = '<span><i class="bi bi-file-earmark-code-fill"></i> JSON</span>';
      jsonExportButton.addEventListener('click', function () {
        ...
        switch (activeTab.textContent.trim()) {
          case 'Earthquake':
            jsonURL = '/spadeapp/earthquake_json/';
            break;
          case 'Space Weather Events':
            jsonURL = '/spadeapp/swe_json/';
            break;
          case 'GRB':
            jsonURL = '/spadeapp/grb_json/';
            break;
          case 'TGF':
            jsonURL = '/spadeapp/tgf_json/';
            break;
          case 'Magnetic disturbances (Dst)':
            jsonURL = '/spadeapp/gms_json/';
            break;
          default:
            jsonURL = '/spadeapp/earthquake_json/';
        }
      ...
      // Insert the jsonExportButton as the first child of tableButtons
      tableButtons.insertBefore(jsonExportButton, tableButtons.firstChild);
    }
  },
  "layout": {
    "topStart": {
...
function enablePage(message, messageType, loadingModal, checkData) {
  ...
  // Start a loop to continuously attempt to close the modal until it's actually closed
  let attemptCloseModal = setInterval(function () {
    if (loadingModal._isShown) {
      loadingModal.hide(); // Attempt to hide the modal if it's shown
    } else {
      console.log('Modal is now closed.'); // Confirm the modal is closed
      clearInterval(attemptCloseModal); // Stop the loop
    }
  }, 100); // Check and attempt to close every 100 milliseconds
...
function loadData() {
  ...
  fetch(myBeautifulURL)
    ...
    .then(data => {
      ...
      // Draw the line chart for the Earthquake data
      drawEQLineChart(data);
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  document.getElementById('openGraphs').addEventListener('click', function () {
    ...
    if (activeTab) {
      let activeTabText = activeTab.textContent.trim();
      let foundTab = false; // Flag to check if the tab has been found and shown
      ...
      // If no specific tab matches, default to showing the "Any" tab
      if (!foundTab) {
        graphsTabs.forEach(tab => {
          if (tab.textContent.trim() === "Any") {
            new bootstrap.Tab(tab).show();
          }
        });
      }
...
function drawGRBChart() {
  const ctx = document.getElementById('grbChart');

  console.log('Drawing the GRB chart');
  ...
  fetch('/spadeapp/graph/?start=' + startDate + '&end=' + endDate + '&theme=' + theme)
    .then(response => response.text())
    .then(data => {
      // put the html data in the div
      document.getElementById('grbChart').innerHTML = data;
      // remove <g id="patch_1"> from the innerhtml
      document.getElementById('patch_1').remove();
      document.getElementById('patch_2').remove();
    })
    .catch(error => {
      console.error('Error:', error);
      showAlert('Error loading GRB chart', 'danger');
    });
}


function drawAnyChart() {
  console.log('Drawing the AnyChart');
  const ctx = document.getElementById('anyChart');

  if (anyChartInstance) {
    anyChartInstance.destroy();
  }
  ...
  async function fetchJSONData() {
    var base_url = "/spadeapp/"

    // urls for all the datasets
    var endpoints = {
      "Earthquakes": "earthquake_json/?start=" + startDate + "&end=" + endDate,
      "GRB": "grb_json/?start=" + startDate + "&end=" + endDate,
      "TGF": "tgf_json/?start=" + startDate + "&end=" + endDate,
      "SWE": "swe_json/?start=" + startDate + "&end=" + endDate,
      "GMS": "gms_json/?start=" + startDate + "&end=" + endDate,
    }
    ...
    for (const [key, value] of Object.entries(endpoints)) {
      ...
      if (!response.ok) {
        ...
        continue; // Skip this dataset on error
      }
      const data = await response.json();
      if (!data.length) {
        console.log(`${key} data is empty`);
      ...
      data.forEach(item => {
        const date = new Date(item.fields.trigger_time).toISOString().split('T')[0];
        if (eventCounts[date] !== undefined) {
          eventCounts[date]++;
        }
      });

      datasets_count[key] = Object.values(eventCounts);
    }
    ...
    var datasets = Object.entries(datasets_count).map(([key, value]) => ({
      label: key,
      data: value,
      borderWidth: 1,
    }));
    ...
    if (!datasets.length) {
      ...
      return; // Exit if no datasets
    }

    anyChartInstance = new Chart(ctx, {
      type: 'bar',
      data: {
        labels: dates,
        datasets: datasets,
